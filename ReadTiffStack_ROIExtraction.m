function [stack_in_raw, filename] = ReadTiffStack_ROIExtraction(filename, img_first, img_last)

%MODIFIED FROM  Francois Nedelec
% nedelec (at) embl.de
...
%Modified to return a single 3d matrix with the third dimension being each
%slice (instead of a data structure with each entry being a slice) and
%added a progress bar and the return of the filename.

%function [stack, img_read, stack_out] = tiffread2stack(filename, img_first, img_last)
...
while (ifd_pos ~= 0)
    %textprogressbar((img_read/img_last)*1000+1);
    clear IMG;
    ...
    if isfield( TIF, 'MM_stack' )
        ...
        %export as single array
        stack_in_raw = [];
        for ii = img_first:img_last
            
            stack_in_raw(:,:,ii) =stack(1,ii).data;
        ...
        if ( img_skip + 1 >= img_first )
            ...
            try
                stack( img_read ) = IMG;
                stack_in_raw(:,:,img_read) =stack(1,img_read).data;
